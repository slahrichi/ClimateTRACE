...
class OutputManager:
    def __init__(
        ...
        self.deleted_path = self.output_dir / f"{stem}_deleted.geojson"
        self.created_path = self.output_dir / f"{stem}_new.geojson"
        self.features: List[Dict[str, Any]] = []
        self.deleted_features: List[Dict[str, Any]] = []
        self.created_features: List[Dict[str, Any]] = []
    ...
    def record(
        ...
        feature = {
            "type": "Feature",
            "geometry": geometry,
            "properties": self._build_properties(building, mask, timestamp, metrics),
        }
        self.features.append(feature)
        if building.properties.get("created"):
            self.created_features.append(feature)
            self.created_ids.add(building_id)
    ...
    def record_deletion(
        ...
        properties = building.properties.copy()
        properties.update(
            {
                "building_id": building_id,
                "source": properties.get("source", "overture"),
                "refined_by": "deleted",
                "timestamp": timestamp,
                "annotator_id": self.annotator_id,
            }
        ...
        if metrics:
            properties.update(
                {
                    "building_elapsed_sec": metrics.get("building_elapsed_sec", 0.0),
                    "image_elapsed_sec": metrics.get("image_elapsed_sec", 0.0),
                    "session_elapsed_sec": metrics.get("session_elapsed_sec", 0.0),
                }
            )
        geometry = mapping(building.polygon)
        self.deleted_features.append({"type": "Feature", "geometry": geometry, "properties": properties})
        self.audit_log.append(
            {
                "timestamp": timestamp,
                "image_path": str(self.session.image_path),
    ...
    def flush(self) -> None:
        ...
        if self.deleted_features:
            with open(self.deleted_path, "w", encoding="utf-8") as f:
                json.dump({"type": "FeatureCollection", "features": self.deleted_features}, f, indent=2)

        if self.created_features:
            with open(self.created_path, "w", encoding="utf-8") as f:
                json.dump({"type": "FeatureCollection", "features": self.created_features}, f, indent=2)
    ...
    def load_existing_progress(self) -> None:
        building_map = {b.feature_id: b for b in self.session.buildings}
        if self.geojson_path.exists():
            with open(self.geojson_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            self.features = data.get("features", [])
            for feature in self.features:
                self._apply_feature(feature, building_map, accepted=True)
        if self.deleted_path.exists():
            with open(self.deleted_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            self.deleted_features = data.get("features", [])
            for feature in self.deleted_features:
                self._apply_feature(feature, building_map, deleted=True)
        if self.created_path.exists():
            with open(self.created_path, "r", encoding="utf-8") as f:
                data = json.load(f)
            self.created_features = data.get("features", [])

    def _apply_feature(
        self,
        feature: Dict[str, Any],
        building_map: Dict[str, BuildingMask],
        accepted: bool = False,
        deleted: bool = False,
    ) -> None:
        ...
        building_id = str(props.get("building_id") or feature.get("id") or "")
        if not building_id:
            return
        ...
        mask = features.rasterize(
            [(polygon, 1)],
            out_shape=(self.session.height, self.session.width),
            transform=self.session.transform,
            fill=0,
            dtype=np.uint8,
        )
        if building is None and not deleted:
            building = self._append_new_building(building_id, polygon, props)
        ...
        if deleted:
            ...
            building.refined_mask = np.zeros_like(mask)

    def _append_new_building(self, building_id: str, polygon: Polygon, props: Dict[str, Any]) -> BuildingMask:
        polygon = _expand_polygon(polygon)
        pixel_exterior, pixel_interiors = polygon_rings_to_pixel_coords(polygon, self.session.transform)
        cols = [p[0] for p in pixel_exterior]
        rows = [p[1] for p in pixel_exterior]
        bbox_col_min = max(0.0, min(cols))
        bbox_col_max = min(float(self.session.width - 1), max(cols))
        bbox_row_min = max(0.0, min(rows))
        bbox_row_max = min(float(self.session.height - 1), max(rows))
        building = BuildingMask(
            idx=len(self.session.buildings),
        ...
        self.session.buildings.append(building)
        return building
    ...
    def _build_properties(
        ...
        props.setdefault("source", "overture")
        props.update(
            {
                "building_id": building.feature_id,
...
class SamAnnotatorApp:
    def __init__(self, entries: List[Tuple[Path, Path]], args: argparse.Namespace, device: str) -> None:
        ...
        self.annotator_id = getattr(args, "annotator_id", None)
        self.session_start_time = time.time()
        self.image_start_time: Optional[float] = None
        self.building_start_time: Optional[float] = None
        ...
        self.create_mode_active = False
        self.create_mask_chip: Optional[np.ndarray] = None
        self.create_points_chip: List[ColorPoint] = []
        self.create_tool_var: Optional[tk.StringVar] = None
        ...
        if not self.annotator_id:
            self.annotator_id = self._prompt_for_annotator()
        self.create_tool_var = tk.StringVar(value="line")
        ...
        self._build_ui()
        self._bind_events()
        self.root.after(0, lambda: self.load_session(0))

    def _prompt_for_annotator(self) -> str:
        annotator = getattr(self.args, "annotator_id", None)
        if annotator:
            return annotator
        dialog = tk.Toplevel(self.root)
        dialog.title("Annotator ID")
        dialog.grab_set()
        ttk.Label(dialog, text="Enter annotator ID:").grid(row=0, column=0, padx=10, pady=10)
        ...
        def submit() -> None:
            ...
            if not value:
                messagebox.showwarning("Annotator ID", "Please enter a non-empty ID.")
                return
            result.append(value)
            dialog.destroy()

        dialog.bind("<Return>", lambda _: submit())
        ttk.Button(dialog, text="OK", command=submit).grid(row=2, column=0, pady=(0, 10))
        self.root.wait_window(dialog)
        return result[0] if result else "annotator"
    ...
    def _build_ui(self) -> None:
        ...
        toolbar_top = ttk.Frame(self.root, padding=(8, 4))
        toolbar_top.grid(row=1, column=0, columnspan=2, sticky="ew")
        for col in range(8):
            toolbar_top.columnconfigure(col, weight=0)
        toolbar_top.columnconfigure(7, weight=1)

        ttk.Radiobutton(toolbar_top, text="Positive (+)", variable=self.mode_var, value="positive").grid(
            row=0, column=0
        )
        ttk.Radiobutton(toolbar_top, text="Negative (-)", variable=self.mode_var, value="negative").grid(
            row=0, column=1
        )
        self.undo_btn = ttk.Button(toolbar_top, text="Undo (z)", command=self.undo_last)
        self.undo_btn.grid(row=0, column=2, padx=(12, 0))
        self.reset_btn = ttk.Button(toolbar_top, text="Reset clicks (r)", command=self.reset_clicks)
        self.reset_btn.grid(row=0, column=3, padx=(6, 0))
        self.resegment_btn = ttk.Button(toolbar_top, text="Re-segment (a)", command=lambda: self.run_sam_async())
        self.resegment_btn.grid(row=0, column=4, padx=(12, 0))
        self.accept_btn = ttk.Button(toolbar_top, text="Accept (s)", command=self.accept_mask)
        self.accept_btn.grid(row=0, column=5, padx=(12, 0))
        self.skip_btn = ttk.Button(toolbar_top, text="Skip (k)", command=self.skip_building)
        self.skip_btn.grid(row=0, column=6, padx=(12, 0))
        self.save_image_btn = ttk.Button(toolbar_top, text="Save & Next (w)", command=self.save_and_next_image)
        self.save_image_btn.grid(row=0, column=7, padx=(12, 0))

        toolbar_bottom = ttk.Frame(self.root, padding=(8, 0))
        toolbar_bottom.grid(row=2, column=0, columnspan=2, sticky="ew")
        for col in range(13):
            toolbar_bottom.columnconfigure(col, weight=0)
        toolbar_bottom.columnconfigure(12, weight=1)

        self.toggle_mask_btn = ttk.Button(toolbar_bottom, text="Toggle Mask (m)", command=self.toggle_mask)
        self.toggle_mask_btn.grid(row=0, column=0, padx=(12, 0), pady=(4, 0))
        self.neighbor_accept_btn = ttk.Button(toolbar_bottom, text="Accept Nearby (h)", command=self.accept_neighbor_masks)
        self.neighbor_accept_btn.grid(row=0, column=1, padx=(12, 0), pady=(4, 0))
        self.delete_btn = ttk.Button(toolbar_bottom, text="Delete (d)", command=self.start_delete_mode)
        self.delete_btn.grid(row=0, column=2, padx=(12, 0), pady=(4, 0))
        self.create_btn = ttk.Button(toolbar_bottom, text="Create (c)", command=self.start_create_mode)
        self.create_btn.grid(row=0, column=3, padx=(12, 0), pady=(4, 0))
        ttk.Label(toolbar_bottom, text="Tool").grid(row=0, column=4, sticky="e", padx=(6, 2))
        ttk.Radiobutton(toolbar_bottom, text="Line", variable=self.create_tool_var, value="line").grid(
            row=0, column=5, padx=(0, 4)
        )
        ttk.Radiobutton(toolbar_bottom, text="Brush", variable=self.create_tool_var, value="brush").grid(
            row=0, column=6, padx=(0, 4)
        )
        self.create_commit_btn = ttk.Button(toolbar_bottom, text="Save New (v)", command=self.commit_create_feature)
        self.create_commit_btn.grid(row=0, column=7, padx=(8, 0), pady=(4, 0))
        self.create_cancel_btn = ttk.Button(toolbar_bottom, text="Cancel New (u / Esc)", command=self.cancel_create_mode)
        self.create_cancel_btn.grid(row=0, column=8, padx=(8, 0), pady=(4, 0))

        self.brush_size = tk.IntVar(value=3)
        self.brush_btn = ttk.Button(toolbar_bottom, text="Brush Edit (b)", command=lambda: self.start_paint_mode("brush"))
        self.brush_btn.grid(row=1, column=0, padx=(12, 0), pady=(4, 0))
        self.eraser_btn = ttk.Button(toolbar_bottom, text="Eraser Edit (e)", command=lambda: self.start_paint_mode("eraser"))
        self.eraser_btn.grid(row=1, column=1, padx=(4, 0), pady=(4, 0))
        ttk.Label(toolbar_bottom, text="Size").grid(row=1, column=2, sticky="e", padx=(8, 2), pady=(4, 0))
        self.brush_spin = tk.Spinbox(toolbar_bottom, from_=1, to=64, width=4, textvariable=self.brush_size)
        self.brush_spin.grid(row=1, column=3, sticky="w", pady=(4, 0))
        self.finish_brush_btn = ttk.Button(toolbar_bottom, text="Done Editing", command=self.finish_paint_mode)
        self.finish_brush_btn.grid(row=1, column=4, padx=(8, 0), pady=(4, 0))
        self.exit_btn = ttk.Button(toolbar_bottom, text="Exit (q)", command=self.finish_and_exit)
        self.exit_btn.grid(row=1, column=5, padx=(12, 0), pady=(4, 0))

        ttk.Label(
            toolbar_bottom,
            text="Scroll/trackpad = zoom, Right/Middle drag = pan, Left-click adds points.",
            foreground="#555",
        ).grid(row=1, column=12, sticky="e", pady=(4, 0))
    ...
    def _bind_events(self) -> None:
        ...
        self.root.bind("c", lambda _: self.start_create_mode())
        self.root.bind("r", lambda _: self.reset_clicks())
        self.root.bind("w", lambda _: self.save_and_next_image())
        self.root.bind("h", lambda _: self.accept_neighbor_masks())
        self.root.bind("v", lambda _: self.commit_create_feature())
        self.root.bind("u", lambda _: self.cancel_create_mode())
        self.root.bind("<Escape>", lambda _: self.cancel_create_mode())
        self.root.bind("b", lambda _: self.start_paint_mode("brush"))
        self.root.bind("e", lambda _: self.start_paint_mode("eraser"))
        self.root.bind("q", lambda _: self.finish_and_exit())
    ...
    def start_paint_mode(self, mode: str) -> None:
        if not self.current_chip or self.sam_running:
            ...
            return
        if self.create_mode_active:
            self.set_status("Finish or cancel creation before editing existing masks.")
            return
    ...
    def _handle_delete_click(self, chip_x: float, chip_y: float) -> None:
        ...
        if self.current_mask_chip.sum() == 0:
            ...
            if self.output_manager is not None:
                ...
                self.output_manager.record_deletion(self.current_building, _now_iso(), metrics=metrics)
    ...
    def start_create_mode(self) -> None:
        if not self.current_chip:
            self.set_status("Load a chip before creating a building.")
            return
        if self.sam_running:
            self.set_status("Wait for SAM to finish before creating a building.")
            return
        self._cancel_delete_mode()
        self.finish_paint_mode()
        self.create_mode_active = True
        self.building_start_time = time.time()
        chip_shape = self.current_chip.display.shape[:2]
        self.create_mask_chip = np.zeros(chip_shape, dtype=np.uint8)
        self.create_points_chip.clear()
        self.canvas.configure(cursor="tcross")
        tool = self.create_tool_var.get()
        if tool not in {"line", "brush"}:
            self.create_tool_var.set("line")
            tool = "line"
        label = "line" if tool == "line" else "brush"
        self.set_status(f"Create mode ({label}) — draw the building and click Save New when done.")
        self.render_canvas()

    def cancel_create_mode(self) -> None:
        if not self.create_mode_active:
            return
        self.create_mode_active = False
        self.create_mask_chip = None
        self.create_points_chip.clear()
        self.canvas.configure(cursor="")
        self.render_canvas()
        self.set_status("Create mode cancelled.")

    def _handle_create_click(self, chip_x: float, chip_y: float) -> None:
        if not self.create_mode_active:
            return
        tool = self.create_tool_var.get()
        if tool == "line":
            self.create_points_chip.append((chip_x, chip_y))
            count = len(self.create_points_chip)
            self.set_status(f"Added vertex {count}. Use Save New to finish or Cancel to restart.")
            self.render_canvas()
        else:
            self._apply_create_brush(chip_x, chip_y)

    def _apply_create_brush(self, chip_x: float, chip_y: float) -> None:
        if not self.create_mode_active or not self.current_chip:
            return
        if self.create_mask_chip is None:
            chip_shape = self.current_chip.display.shape[:2]
            self.create_mask_chip = np.zeros(chip_shape, dtype=np.uint8)
        mask = self.create_mask_chip
        radius = max(1, int(self.brush_size.get()))
        row = int(round(chip_y))
        col = int(round(chip_x))
        h, w = mask.shape
        ...
        patch[circle] = 1
        mask[r0:r1, c0:c1] = patch
        self.create_mask_chip = mask
        self.set_status(f"Create mask pixels: {int(mask.sum())}.")
        self.render_canvas()

    def commit_create_feature(self) -> None:
        if not self.create_mode_active:
            self.set_status("Start create mode first.")
            return
        if not self.current_chip or not self.session:
            self.set_status("No session loaded.")
            return
        tool = self.create_tool_var.get()
        if tool == "line":
            mask = self._create_polygon_mask()
        else:
            mask = self.create_mask_chip.copy() if self.create_mask_chip is not None else None
        if mask is None or not mask.any():
            self.set_status("Draw a polygon or brush region before saving the new building.")
            return
        try:
            feature_id = self._finalize_created_building(mask)
        except ValueError as exc:
            self.set_status(str(exc))
            return
        self.cancel_create_mode()
        self.set_status(f"Added new building {feature_id}.")

    def _create_polygon_mask(self) -> Optional[np.ndarray]:
        if len(self.create_points_chip) < 3 or not self.current_chip:
            return None
        h, w = self.current_chip.display.shape[:2]
        poly_pts = [(float(x), float(y)) for x, y in self.create_points_chip]
    ...
    def _finalize_created_building(self, chip_mask: np.ndarray) -> str:
        if not self.current_chip or not self.session:
            raise ValueError("No active chip for creation.")
        chip_mask = (chip_mask > 0).astype(np.uint8)
        if not chip_mask.any():
            raise ValueError("New building mask is empty.")
        ...
        shapes_iter = features.shapes(chip_mask.astype(np.uint8), transform=chip.transform)
        polygons = []
        for geom, val in shapes_iter:
            ...
            if polygon.area > 0:
                polygons.append(polygon)
        if not polygons:
            raise ValueError("Unable to derive polygon from the drawn mask.")
        geom = unary_union(polygons).buffer(0)
        polygon = _expand_polygon(geom)
        pixel_exterior, pixel_interiors = polygon_rings_to_pixel_coords(polygon, self.session.transform)
        cols = [p[0] for p in pixel_exterior]
        rows = [p[1] for p in pixel_exterior]
        bbox_col_min = max(0.0, min(cols))
        ...
        feature_id = f"user_building_{self.created_counter:04d}"
        self.created_counter += 1

        building = BuildingMask(
            idx=len(self.session.buildings),
            feature_id=feature_id,
            properties={"source": "user", "created": True},
            polygon=polygon,
            pixel_polygon=pixel_exterior,
            pixel_polygon_interiors=pixel_interiors,
            bbox_row_min=bbox_row_min,
            ...
            bbox_col_min=bbox_col_min,
            bbox_col_max=bbox_col_max,
            refined_mask=global_mask,
            accepted=True,
        )
        self.session.buildings.append(building)
        metrics = self._current_metrics()
        self.output_manager.record(building, global_mask, _now_iso(), metrics=metrics)
        self.building_list.insert(tk.END, f"[✓] {feature_id}")
        self.goto_building(building.idx)
        return feature_id

    def _current_metrics(self) -> Dict[str, float]:
        now = time.time()
        building_elapsed = now - self.building_start_time if self.building_start_time else 0.0
        image_elapsed = now - self.image_start_time if self.image_start_time else 0.0
        session_elapsed = now - self.session_start_time
        return {
            "building_elapsed_sec": float(building_elapsed),
            "image_elapsed_sec": float(image_elapsed),
            "session_elapsed_sec": float(session_elapsed),
        }
    ...
    def load_session(self, index: int) -> None:
        ...
        self.output_manager.load_existing_progress()
        self._reindex_buildings()
        self._sync_created_counter()
        ...
        start_idx = 0
        for idx, candidate in enumerate(self.session.buildings):
            if not candidate.accepted and not candidate.deleted:
                start_idx = idx
                break
        self.goto_building(start_idx)
    ...
    def _reindex_buildings(self) -> None:
        if not self.session:
            return
        for idx, building in enumerate(self.session.buildings):
            building.idx = idx

    def _sync_created_counter(self) -> None:
        max_idx = self.created_counter
        for building in self.session.buildings:
            fid = building.feature_id
            if fid.startswith("user_building_"):
                try:
                    max_idx = max(max_idx, int(fid.split("_")[-1]) + 1)
                except ValueError:
                    continue
        self.created_counter = max_idx
    ...
    def render_canvas(self) -> None:
        ...
        if self.create_mode_active:
            if self.create_mask_chip is not None and self.create_mask_chip.any():
                create_img = Image.fromarray((self.create_mask_chip > 0).astype(np.uint8) * 255, mode="L")
                create_overlay = Image.new("RGBA", base.size, (0, 255, 255, 120))
                base = Image.composite(create_overlay, base, create_img)
            if self.create_points_chip:
                pts = [(p[0], p[1]) for p in self.create_points_chip]
                if len(pts) >= 2:
                    draw.line(pts, fill=(255, 255, 0, 220), width=2)
                for x, y in pts:
                    draw.ellipse((x - 2, y - 2, x + 2, y + 2), fill=(255, 255, 0, 220))
    ...
    def on_canvas_click(self, event: tk.Event) -> None:
        ...
        if self.delete_mode:
            ...
            return
        if self.create_mode_active:
            self._handle_create_click(chip_x, chip_y)
            return
        if self.paint_mode:
    ...
    def on_canvas_drag(self, event: tk.Event) -> None:
        if self.create_mode_active:
            if self.create_tool_var.get() == "brush":
                canvas_x = self.canvas.canvasx(event.x)
                canvas_y = self.canvas.canvasy(event.y)
                chip_x = canvas_x / self.zoom
                chip_y = canvas_y / self.zoom
    ...
    def accept_mask(self) -> None:
        ...
        timestamp = _now_iso()
        metrics = self._current_metrics()
        self.total_active_seconds = metrics["session_elapsed_sec"]
        self.output_manager.record(self.current_building, global_mask, timestamp, metrics=metrics)
        self.output_manager.flush()
        self._update_building_entry(self.current_building.idx, "✓")
    ...
    def accept_neighbor_masks(self) -> None:
        ...
        for building in neighbors:
            ...
            building.skipped = False
            metrics = self._current_metrics()
            metrics["building_elapsed_sec"] = 0.0
            self.output_manager.record(building, mask, _now_iso(), metrics=metrics)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--annotator-id",
        type=str,
        help="Identifier for the annotator; stored in GeoJSON properties and audit logs.",
    )
...
def determine_entries(args: argparse.Namespace) -> List[Tuple[Path, Path]]:
    ...
    raise ValueError("Provide either --image/--annotations, or --images-dir/--masks-dir.")
